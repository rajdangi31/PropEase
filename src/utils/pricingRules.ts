...
export function calculateRentEstimate(attributes: PropertyPricingAttributes) {
  ...
  if (furnishedStatus === "fully-furnished") {
    ...
    factors.push({ name: "Fully Furnished", adjustmentPercentage: 10, type: "positive", reason: "Fully furnished properties command a premium." });
  } else if (furnishedStatus === "semi-furnished") {
    totalMultiplier += 0.05;
    factors.push({ name: "Semi Furnished", adjustmentPercentage: 5, type: "positive", reason: "Partially furnished properties add convenience." });
  }

  // 3. Parking
  if (parking) {
    totalMultiplier += 0.05;
    factors.push({ name: "Parking Available", adjustmentPercentage: 5, type: "positive", reason: "Dedicated parking is a valuable amenity." });
  }
  ...
  if (balconyCount > 0) {
    ...
    factors.push({ name: `${balconyCount} Balcon${balconyCount > 1 ? 'ies' : 'y'}`, adjustmentPercentage: Math.round(balconyBoost * 100), type: "positive", reason: "Outdoor space increases desirability." });
  }

  // 5. Transit
  if (nearMetro) {
    totalMultiplier += 0.08;
    factors.push({ name: "Near Metro/Transit", adjustmentPercentage: 8, type: "positive", reason: "Excellent transit access drives higher demand." });
  }

  // 6. Location Intelligence
  if (locationScore > 85) {
    totalMultiplier += 0.10;
    factors.push({ name: "High Location Score", adjustmentPercentage: 10, type: "positive", reason: "Located in a highly sought-after neighborhood." });
  } else if (locationScore >= 70) {
    totalMultiplier += 0.05;
    factors.push({ name: "Good Location Score", adjustmentPercentage: 5, type: "positive", reason: "Solid neighborhood amenities." });
  }

  // 7. Property Age
  if (propertyAge > 20) {
    totalMultiplier -= 0.10;
    factors.push({ name: `Older Property (${propertyAge} yrs)`, adjustmentPercentage: 10, type: "negative", reason: "Older properties generally have lower rents." });
  } else if (propertyAge > 10) {
    totalMultiplier -= 0.05;
    factors.push({ name: `Aging Property (${propertyAge} yrs)`, adjustmentPercentage: 5, type: "negative", reason: "Slight discount for property age." });
  }
...
export function generateExplanation(factors: PricingFactor[], finalEstimate: number): string[] {
  const sortedFactors = [...factors].sort((a, b) => b.adjustmentPercentage - a.adjustmentPercentage);
