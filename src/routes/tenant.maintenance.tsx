...
import { Camera, Plus, Calendar, Shield, User, MessageSquare, Send, Loader2, CheckCircle, AlertCircle, HelpCircle, Wrench } from "lucide-react";
...
import { getMyMaintenanceAsTenantFn, createMaintenanceRequestFn, getMaintenanceLogsFn, addMaintenanceLogFn } from "@/lib/data-server";
...
function TenantMaintenance() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await createMaintenanceRequestFn({ data: { title, description: `${category}: ${description}`, priority, category } });
        ...
        <CardContent>
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div className="space-y-1.5">
              ...
              <Input placeholder="e.g. Kitchen faucet leaking" value={title} onChange={e => setTitle(e.target.value)} required />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                <Label>Urgency</Label>
                <select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={priority} onChange={e => setPriority(e.target.value)}>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea rows={5} placeholder="Tell us what's happening..." value={description} onChange={e => setDescription(e.target.value)} required />
            </div>
          ...
          {requests.length === 0 ? (
            ...
            requests.map((r: any) => (
              ...
              >
                ...
                <p className="mt-1.5 text-[10px] text-muted-foreground">Submitted {r.submitted?.split(" ")[0] || r.date?.split(" ")[0]}</p>
              </button>
            ))
          )}
        </CardContent>
            ...
            <div className="flex flex-col h-[600px] max-h-[85vh]">
              ...
              <div className="flex items-center justify-between border-b border-border/50 px-5 py-4 bg-muted/10">
                <div className="min-w-0">
                  ...
                  <DialogTitle className="mt-2 text-lg font-bold leading-tight font-display text-foreground truncate max-w-[600px]" title={selectedRequest.title}>
              ...
              <div className="flex-1 grid grid-cols-1 md:grid-cols-12 overflow-hidden">
                ...
                <div className="md:col-span-5 p-5 overflow-y-auto space-y-4 border-r border-border/50">
                  <div className="bg-muted/30 border border-border/40 p-4 rounded-xl space-y-2">
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      ...
                      <span>Submitted on <strong>{selectedRequest.submitted?.split(" ")[0]}</strong></span>
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Assigned Worker</Label>
                    <div className="text-xs font-medium text-foreground bg-muted/20 px-3 py-1.5 rounded-md border border-border/30 flex items-center gap-1.5">
                      <User className="h-3.5 w-3.5 text-accent" /> {selectedRequest.assigned ?? "Unassigned"}
                    </div>
                  </div>
                ...
                <div className="md:col-span-7 flex flex-col h-full bg-muted/5 overflow-hidden">
                        ...
                        <div key={log.id} className="flex flex-col gap-1">
                          <div className="flex items-baseline justify-between gap-2 px-1">
                            <span className="text-[10px] font-semibold text-muted-foreground">
                              {log.authorName} <span className="opacity-80 font-normal">({log.authorRole})</span>
                            </span>
                            <span className="text-[9px] text-muted-foreground opacity-80">
                              {log.createdAt.split(" ")[1]?.substring(0, 5) || log.createdAt.split(" ")[0]}
                            </span>
                          </div>
                  ...
                  {/* Comment Input */}
                  <form onSubmit={handleAddComment} className="p-4 border-t border-border/50 bg-card">
