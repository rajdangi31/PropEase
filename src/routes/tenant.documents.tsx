...
import { Download, FileText, Upload, Calendar, Loader2, CheckCircle, AlertCircle, HelpCircle } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground font-medium">Manage and upload your lease documents, ID proofs, and income verification files</p>
        </div>
        ...
        <Card className="border-dashed border-border/60 bg-card/10">
          <CardContent className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="mt-2 max-w-sm text-sm text-muted-foreground mb-6">
              Upload your verification proofs (ID/Income) or sign lease documents to keep them organized here.
            </p>
            <Button onClick={() => setUploadOpen(true)}>
              ...
              <TableBody>
                {documents.map((doc) => (
                  <TableRow key={doc.id} className="hover:bg-muted/5 transition-colors">
                    ...
                    <TableCell>
                      <Badge variant="outline" className={`gap-1 font-medium capitalize py-0.5 px-2.5 rounded-full ${statusStyles[doc.status] || ""}`}>
                        {doc.status === "approved" ? (
      ...
      {/* Upload Dialog */}
      <Dialog open={uploadOpen} onOpenChange={(open) => !open && (setUploadOpen(false), setSelectedFile(null))}>
        <DialogContent className="sm:max-w-md">
          <form onSubmit={handleUploadSubmit}>
            <DialogHeader>
              <DialogTitle>Upload Verification Document</DialogTitle>
              <DialogDescription>
                Select the type of document you want to upload and choose the file. Only PDF, JPG/JPEG, PNG, and Word files are supported.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={docType} onValueChange={(val) => setDocType(val as DocType)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="income_proof">Income Verification (e.g. Paystubs, W2)</SelectItem>
